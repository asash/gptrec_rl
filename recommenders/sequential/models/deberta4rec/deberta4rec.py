...
class Deberta4Rec(SequentialRecsysModelBuilder):
    ...
    def get_model(self):
        ...
        return Deberta4RecModel(self.batch_size, self.output_layer_activation, deberta_config, self.max_history_length)


class Deberta4RecModel(Model):
    def __init__(self, batch_size, outputput_layer_activation, deberta_config, sequence_length, 
                        *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.batch_size = batch_size
        self.output_layer_activation = outputput_layer_activation
        self.token_type_ids = tf.constant(tf.zeros(shape=(batch_size, deberta_config.max_position_embeddings)))
