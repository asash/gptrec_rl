...
class BertJPQConfig(SequentialModelConfig):
    def __init__(self,
                 embedding_size = 64, 
    ...
    def get_model_architecture(self) -> Type[BertJPQModel]:
        return BertJPQModel
        


class BertJPQModel(SequentialRecsysModel):
    def __init__(self, model_parameters: BertJPQConfig, data_parameters: SequentialDataParameters, *args, **kwargs):
        super().__init__(model_parameters, data_parameters, *args, **kwargs)
        self.model_parameters: BertJPQConfig
    ...
    @classmethod
    def get_model_config_class(cls) -> Type[BertJPQConfig]:
        return BertJPQConfig
    ...
    def call(self, inputs, **kwargs):
        ...
        item_embeddings = self.item_codes_layer(masked_sequences, self.data_parameters.batch_size)
        bert_output = self.bert(None, inputs_embeds=item_embeddings, position_ids = positions).last_hidden_state
        logits = self.item_codes_layer.score_sequence_all_items(bert_output, self.data_parameters.batch_size) 
